...
function Lens_ParseArgs($args, $endstmtchar = ',', $tokenchars = '_.-')
{
    ...
    while ($pos < $max) {
        ...
        default:
            ...
            if ($quoted) {
                $tokarr[] = $ch;
            } elseif (ctype_alnum($ch) || false !== strpos($tokenchars, $ch)) {
                $tokarr[] = $ch;
            } else {
                if ($ch == $endstmtchar) {
                    $tokens[$stmtno][] = implode('', $tokarr);
    ...
    */
    ...
    public function _GenFields($flds, $widespacing = false)
    {
        ...
        foreach ($flds as $fld) {
            ...
            // CONSTRUCT FIELD SQL
            if ($fdefts) {
                if ('mysql' == substr($this->connection->databaseType, 0, 5)) {
                    $ftype = 'TIMESTAMP';
                } else {
            ...
            } elseif (false !== $fdefault && !$fnoquote) {
                if ('C' == $ty or 'X' == $ty or
                    ("'" != substr($fdefault, 0, 1) && !is_numeric($fdefault))) {
                    if (('D' == $ty || 'T' == $ty) && 'null' != strtolower($fdefault)) {
                    ...
                    } elseif (1 != strlen($fdefault) && ' ' == substr($fdefault, 0, 1) && ' ' == substr($fdefault, strlen($fdefault) - 1)) {
                        $fdefault = trim($fdefault);
                    } elseif ('null' != strtolower($fdefault)) {
    ...
    {
        if (strlen($fsize) && 'X' != $ty && 'B' != $ty && false === strpos($ftype, '(')) {
            $ftype .= '('.$fsize;
            if (strlen($fprec)) {
                $ftype .= ','.$fprec;
            }
    ...
    public function ChangeTableSQL($tablename, $flds, $tableoptions = false, $dropOldFlds = false)
    {
        ...
        if (false !== $this->connection->fetchMode) {

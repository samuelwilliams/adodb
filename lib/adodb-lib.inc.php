...
function _adodb_replace(&$zthis, $table, $fieldArray, $keyCol, $autoQuote, $has_autoinc)
{
    ...
    foreach ($fieldArray as $k => $v) {
        if (null === $v) {
            $v = 'NULL';
            $fieldArray[$k] = $v;
        } elseif ($autoQuote && /*!is_numeric($v) /*and strncmp($v,"'",1) !== 0 -- sql injection risk*/ 0 != strcasecmp($v, $zthis->null2null)) {
            $v = $zthis->qstr($v);
            $fieldArray[$k] = $v;
        }
    ...
    while (!$zthis->EOF) {
        ...
        */
        ...
        if (is_array($defstr)) {
            ...
            if (0 == strcasecmp($selected, $defstr)) {
                $s .= "\n<option selected='selected'$value>".htmlspecialchars($zval).'</option>';
            } else {
                $s .= "\n<option".$value.'>'.htmlspecialchars($zval).'</option>';
            }
...
function _adodb_getcount(&$zthis, $sql, $inputarr = false, $secs2cache = 0)
{
    ...
    if (!empty($zthis->_nestedSQL) || preg_match("/^\s*SELECT\s+DISTINCT/is", $sql) ||
        ...
        } elseif (0 == strncmp($zthis->databaseType, 'postgres', 8) || 0 == strncmp($zthis->databaseType, 'mysql', 5)) {
    ...
    for ($i = 0, $max = $rs->FieldCount(); $i < $max; ++$i) {
        ...
        if (adodb_key_exists($upperfname, $arrFields, $force)) {
            ...
            if ($forceUpdate || strcmp($val, $arrFields[$upperfname])) {
                ...
                if ((false !== strpos($upperfname, ' ')) || ($ADODB_QUOTE_FIELDNAMES)) {
                ...
                if (is_null($arrFields[$upperfname])
                    || (empty($arrFields[$upperfname]) && 0 == strlen($arrFields[$upperfname]))

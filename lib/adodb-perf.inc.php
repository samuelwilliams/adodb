...
function adodb_getmem()
{
    ...
    if (0 == strncmp(strtoupper(PHP_OS), 'WIN', 3)) {
        $output = array();
...
function adodb_log_sql(&$connx, $sql, $inputarr)
{
    ...
    if (!empty($connx->_logsql) && (empty($connx->_logsqlErrors) || !$rs)) {
        ...
        if (!$rs) {
            ...
            $dbg = $conn->debug;
            $conn->debug = false;
            if (!is_object($rs) || 'empty' == $rs->dataProvider) {
        ...
        if ('oci8' == $conn->dataProvider && 'oci8po' != $dbT) {
            $isql = "insert into $perf_table values($d,:b,:c,:d,:e,:f)";
        } elseif ('odbc_mssql' == $dbT || 'informix' == $dbT || 0 == strncmp($dbT, 'odbtp', 4)) {
            $timer = $arr['f'];
            if ('informix' == $dbT) {
...
<table border=1 bgcolor=white><tr><td><b>Avg Time</b><td><b>Count</b><td><b>SQL</b><td><b>Max</b><td><b>Min</b></tr>\n";
        ...
        while (!$rs->EOF) {
            ...
            $suffix = '</a>';
            if (false == $this->explain || strlen($prefix) > $max) {

...
class dbData extends dbObject
{
    ...
    public function create(&$xmls)
    {
        ...
        foreach ($this->data as $row) {
            ...
            if (!in_array($table, $tables) or XMLS_MODE_INSERT == ($mode = $xmls->existingData())) {
            ...
            if (empty($ukeys) or 0 == count($keyFields)) {
...
class adoSchema
{
    ...
    public function SetPrefix($prefix = '', $underscore = true)
    {
        ...
        if ($underscore and '_' != substr($prefix, -1)) {
            $prefix .= '_';
        }
    ...
    public function addSQL($sql = null)
    {
        ...
        if (is_string($sql)) {
            ...
            if ($this->ExecuteInline() && (2 == $this->success || $this->ContinueOnError())) {
                $saved = $this->db->debug;

...
class ADODB2_postgres extends ADODB_DataDict
{
    ...
    public function MetaType($t, $len = -1, $fieldobj = false)
    {
        ...
        $is_serial = is_object($fieldobj) && $fieldobj->primary_key && $fieldobj->unique &&
            $fieldobj->has_default && 'nextval(' == substr($fieldobj->default_value, 0, 8);
    ...
    public function _recreate_copy_table($tabname, $dropflds, $tableflds, $tableoptions = '')
    {
        ...
        foreach ($this->MetaIndexes($tabname) as $idx_name => $idx_data) {
            if ('_pkey' != substr($idx_name, -5) && (!$dropflds || !count(array_intersect($dropflds, $idx_data['columns'])))) {
...
[ WHERE predicate ]
    */
    ...
    {
        if (strlen($fsize) && 'X' != $ty && 'B' != $ty && 'I' != $ty && false === strpos($ftype, '(')) {
            $ftype .= '('.$fsize;
            if (strlen($fprec)) {
                $ftype .= ','.$fprec;
            }

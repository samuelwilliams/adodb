...
if (!defined('_ADODB_CSV_LAYER')) {
    ...
    class ADODB_csv extends ADOConnection
    {
        ...
        {
            if ('http://' !== strtolower(substr($argHostname, 0, 7))) {
                return false;
            }
        ...
        public function SelectLimit($sql, $nrows = -1, $offset = -1)
        {
            ...
            ((false !== $this->fetchMode) ? $this->fetchMode : $ADODB_FETCH_MODE).
            "&offset=$offset";
            $err = false;
            ...
            if (is_object($rs)) {
                $rs->databaseType = 'csv';
                $rs->fetchMode = (false !== $this->fetchMode) ? $this->fetchMode : $ADODB_FETCH_MODE;
                $rs->connection = $this;
            }

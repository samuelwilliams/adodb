...
class ADODB_ibase extends ADOConnection
{
    ...
    public function _connect($argHostname, $argUsername, $argPassword, $argDatabasename, $persist = false)
    {
        ...
        if (1 != $this->dialect) { // http://www.ibphoenix.com/ibp_60_del_id_ds.html
            $this->replaceQuote = "''";
        }
        ...
        switch ($ftype) {
            ...
            case 8:
                if ($dialect3) {
                    switch ($fsubtype) {
                        case 0:
                            $fld->type = (7 == $ftype ? 'smallint' : 'integer');
                            break;
                        case 1:
                            $fld->type = 'numeric';
    ...
    public function MetaColumns($table, $normalize = true)
    {
        ...
        $retarr = array();
        //OPN STUFF start
        $dialect3 = (3 == $this->dialect ? true : false);
        //OPN STUFF end
        while (!$rs->EOF) { //print_r($rs->fields);
            ...
            if ((isset($rs->fields[5])) && ('blob' == $fld->type)) {
    ...
    public function UpdateBlobFile($table, $column, $path, $where, $blobtype = 'BLOB')
    {
        ...
        return false != $this->Execute("UPDATE $table SET $column=(?) WHERE $where", array($blob_id_str));
    }
...
class ADORecordset_ibase extends ADORecordSet
{
    ...
    public function ADORecordset_ibase($id, $mode = false)
    {
        ...
        $this->fetchMode = (false === $mode) ? $ADODB_FETCH_MODE : $mode;
    ...
    public function _fetch()
    {
        ...
        $this->fields = $f;
        if (ADODB_FETCH_ASSOC == $this->fetchMode) {
            $this->fields = $this->GetRowAssoc(ADODB_ASSOC_CASE);
        } elseif (ADODB_FETCH_BOTH == $this->fetchMode) {

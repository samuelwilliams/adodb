...
order by constraint_name, referenced_table_name, keyno";
    ...
    {
        if ($this->_logsql && false !== $this->_errorCode) {
        ...
        if (is_array($inputarr)) {
            ...
            foreach ($inputarr as $v) {
                ...
                if (is_string($v)) {
                    ...
                    $params .= "@P$i=N".(0 == strncmp($v, "'", 1) ? $v : $this->qstr($v));
                } elseif (is_integer($v)) {
                    $decl .= "@P$i INT";
                    $params .= "@P$i=".$v;
                } elseif (is_float($v)) {
...
class ADORecordset_mssql extends ADORecordSet
{
    ...
    {
        if (ADODB_FETCH_NUM != $this->fetchMode) {

    ...
    public function ADODB_mysql()
    {
        ...
        public function MetaIndexes($table, $primary = false, $owner = false)
        {
            ...
            while ($row = $rs->FetchRow()) {
                if (false == $primary and 'PRIMARY' == $row[2]) {
                    continue;
                }
        ...
        public function GetOne($sql, $inputarr = false)
        {
            ...
            if (false == $this->compat323 && 0 == strncasecmp($sql, 'sele', 4)) {
        ...
        public function MetaColumns($table, $normalize = true)
        {
            ...
            while (!$rs->EOF) {
                ...
                $fld->not_null = ('YES' != $rs->fields[2]);
                $fld->primary_key = ('PRI' == $rs->fields[3]);
                $fld->auto_increment = (false !== strpos($rs->fields[5], 'auto_increment'));
                $fld->binary = (false !== strpos($type, 'blob') || false !== strpos($type, 'binary'));
                $fld->unsigned = (false !== strpos($type, 'unsigned'));
                $fld->zerofill = (false !== strpos($type, 'zerofill'));
        ...
        */
        ...
        {
            global $ADODB_FETCH_MODE;
            if (ADODB_FETCH_ASSOC == $ADODB_FETCH_MODE || ADODB_FETCH_ASSOC == $this->fetchMode) {
                $associative = true;
            }
    ...
    class ADORecordSet_mysql extends ADORecordSet
    {
        ...
        public function FetchField($fieldOffset = -1)
        {
            if (-1 != $fieldOffset) {
                ...
                if ($o) {
                    $o->binary = (false !== strpos($f, 'binary'));
                }
            } elseif (-1 == $fieldOffset) {	/*	The $fieldOffset argument is not provided thus its -1 	*/
        ...
        {
            if (MYSQL_ASSOC == $this->fetchMode && !$upper) {
                $row = $this->fields;
            } else {

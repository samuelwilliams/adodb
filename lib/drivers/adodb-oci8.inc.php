...
";
    ...
    public function _connect($argHostname, $argUsername, $argPassword, $argDatabasename, $mode = 0)
    {
        ...
        if ($argHostname) { // added by Jorma Tuomainen <jorma.tuomainen@ppoy.fi>
            if (empty($argDatabasename)) {
                ...
                if (0 == strncasecmp($argDatabasename, 'SID=', 4)) {
    ...
    */
    public function SelectLimit($sql, $nrows = -1, $offset = -1, $inputarr = false, $secs2cache = 0)
    {
        ...
        if (-1 == $offset || ($offset < $this->selectOffsetAlg1 && 0 < $nrows && $nrows < 1000)) {
            if ($nrows > 0) {
                if ($offset > 0) {
                    $nrows += $offset;
                }
                //$inputarr['adodb_rownum'] = $nrows;
                if ('oci8po' == $this->databaseType) {
            ...
            if (is_array($inputarr)) {
                foreach ($inputarr as $k => $v) {
                    if (is_array($v)) {
                        if (2 == sizeof($v)) { // suggested by g.giunta@libero.
            ...
            ++$offset; // in Oracle rownum starts at 1
...
return false;
    ...
    */
    public function Prepare($sql, $cursor = false)
    {
        ...
        if ('BEGIN' == $sttype || 'DECLARE' == $sttype) {
    ...
    */
    public function Bind(&$stmt, &$var, $size = 4000, $type = false, $name = false, $isOutput = false)
    {
        ...
        if ((OCI_B_CURSOR == $type) && sizeof($stmt) >= 5) {
        ...
        if (OCIExecute($stmt, $this->_commit)) {
            ...
            if (count($this->_refLOBs) > 0) {
                foreach ($this->_refLOBs as $key => $value) {
                    if (true == $this->_refLOBs[$key]['TYPE']) {
...
"select constraint_name,r_owner,r_constraint_name 
    ...
    public function qstr($s, $magic_quotes = false)
    {
        ...
        if ($this->noNullStrings && 0 == strlen($s)) {
            $s = ' ';
        }
        if (!$magic_quotes) {

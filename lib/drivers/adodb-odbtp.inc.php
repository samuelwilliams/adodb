...
class ADODB_odbtp extends ADOConnection
{
    ...
    */
    ...
    public function CreateSequence($seqname = 'adodbseq', $start = 1)
    {
        ...
        $seqtab = 'adodb_seq';
        if (ODB_DRIVER_FOXPRO == $this->odbc_driver) {
    ...
    {
        if ($argPassword && false !== stripos($UserOrDSN, 'DRIVER=')) {
        ...
        if (ODB_DRIVER_ORACLE == $this->odbc_driver) {
            $dbms = 'oracle';
        } elseif (ODB_DRIVER_SYBASE == $this->odbc_driver) {
            $dbms = 'sybase';
        }
    ...
    */
    ...
    {
        if (ODB_DRIVER_JET == $this->odbc_driver) {
            $name = '['.$name.']';
            if (!$type && $this->_useUnicodeSQL
                && ODB_CHAR == @odbtp_param_bindtype($stmt[1], $name)) {
                $type = ODB_WCHAR;
            }
        } else {
            $name = '@'.$name;
    ...
    */
    ...
    public function UpdateBlob($table, $column, $val, $where, $blobtype = 'image')
    {
        ...
        return false != @odbtp_execute($stmt);
    }

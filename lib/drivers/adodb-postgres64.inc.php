...
function adodb_addslashes($s)
{
    ...
    if (0 === strncmp($s, "'", 1) && "'" == substr($s, $len - 1)) {
        return $s;
    } // already quoted
...
WHERE relkind in ('r','v') AND (c.relname='%s' or c.relname = lower('%s'))
 and c.relnamespace=n.oid and n.nspname='%s' 
    ...
    {
        if (!is_resource($this->_resultid) || 'pgsql result' !== get_resource_type($this->_resultid)) {
            return false;
        }
    ...
    */
    ...
    public function MetaColumns($table, $normalize = true)
    {
        ...
        if (!empty($this->metaDefaultsSQL)) {
            ...
            if ($rsdef) {
                while (!$rsdef->EOF) {
                    ...
                    $s = $rsdef->fields['def'];
                    if (false === strpos($s, '::') && "'" == substr($s, 0, 1)) { /* quoted strings hack... for now... fixme */
                        $s = substr($s, 1);
        ...
        while (!$rs->EOF) {
            ...
            $fld->has_default = ('t' == $rs->fields[5]);
            if ($fld->has_default) {
                $fld->default_value = $rsdefa[$rs->fields[6]];
            }

            //Freek
            $fld->not_null = 't' == $rs->fields[4];

            // Freek
            if (is_array($keys)) {
                foreach ($keys as $key) {
                    if ($fld->name == $key['column_name'] and 't' == $key['primary_key']) {
                        $fld->primary_key = true;
                    }
                    if ($fld->name == $key['column_name'] and 't' == $key['unique_key']) {
                        $fld->unique = true;
...
WHERE (c2.relname=\'%s\' or c2.relname=lower(\'%s\'))';
    ...
    public function _connect($str, $user = '', $pwd = '', $db = '', $ctype = 0)
    {
        ...
        if (1 === $ctype) { // persistent
            $this->_connectionID = pg_pconnect($str);
        } else {
            if (-1 === $ctype) { // nconnect, we trick pgsql ext by changing the connection str
                static $ncnt;
...
class ADORecordSet_postgres64 extends ADORecordSet
{
    ...
    {
        if (PGSQL_ASSOC == $this->fetchMode && !$upper) {
            return $this->fields;
        }
    ...
    {
        if (PGSQL_NUM == $this->fetchMode || PGSQL_BOTH == $this->fetchMode) {
            foreach ($this->_blobArr as $k => $v) {
                $this->fields[$k] = ADORecordSet_postgres64::_decode($this->fields[$k]);
            }
        }
        if (PGSQL_ASSOC == $this->fetchMode || PGSQL_BOTH == $this->fetchMode) {
            foreach ($this->_blobArr as $k => $v) {

    ...
    public $hasInsertId = true;
        ...
        public function MetaIndexes($table, $primary = false, $owner = false)
        {
            ...
            while ($row = $rs->FetchRow()) {
                $indexes[$row[0]]['unique'] = 'UNIQUE' == $row[1];
        ...
        public function MetaColumns($table)
        {
            ...
            foreach ($this->GetAll("SELECT COLUMNNAME,DATATYPE,LEN,DEC,NULLABLE,MODE,\"DEFAULT\",CASE WHEN \"DEFAULT\" IS NULL THEN 0 ELSE 1 END AS HAS_DEFAULT FROM COLUMNS WHERE tablename=$table ORDER BY pos") as $column) {
                ...
                $fld->name = $column[0];
                $fld->type = $column[1];
                $fld->max_length = 'LONG' == $fld->type ? 2147483647 : $column[2];
                $fld->scale = $column[3];
                $fld->not_null = 'NO' == $column[4];
                $fld->primary_key = 'KEY' == $column[5];
                if ($fld->has_default = $column[7]) {
                    if ($fld->primary_key && 'DEFAULT SERIAL (1)' == $column[6]) {

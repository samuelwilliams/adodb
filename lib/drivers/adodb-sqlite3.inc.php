...
class ADODB_sqlite3 extends ADOConnection
{
    ...
    public function _connect($argHostname, $argUsername, $argPassword, $argDatabasename) //**tochange: all the function need to be changed, just hacks for the moment
    {
    ...
    // returns true or false
    public function _pconnect($argHostname, $argUsername, $argPassword, $argDatabasename) //**tochange
    {
        if (!function_exists('sqlite_open')) {
            return null;
        }
    ...
    */
    ...
    public function MetaIndexes($table, $primary = false, $owner = false, $owner = false)
    {
        ...
        while ($row = $rs->FetchRow()) {
            if ($primary && 0 == preg_match('/primary/i', $row[1])) {
                continue;
            }

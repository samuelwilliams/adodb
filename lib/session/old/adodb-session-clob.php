...
-- Session: connection failed</p>', false);
    ...
    $expiry = time() + $ADODB_SESS_LIFE;
        ...
        if (false !== $ADODB_SESSION_CRC && $ADODB_SESSION_CRC == strlen($val).crc32($val)) {
            if ($ADODB_SESS_DEBUG) {
                echo '
-- Session: Only updating date - crc32 not changed</p>';
        ...
        if (false === $ADODB_SESSION_USE_LOBS) {	// no lobs, simply use replace()
...
-- Session Replace: '.nl2br($err).'</p>', false);
            ...
            if ('access' == $ADODB_SESS_CONN->databaseType) {
...
-- <b>Garbage Collection</b>: $qry</p>");
        ...
        if ('oci8' === $ADODB_SESS_CONN->dataProvider) {

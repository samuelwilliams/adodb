...
//	$person->HasMany('children','person_id');  ## this is affects all other instances of Person
    ...
    $c = $person2->children;
    if (is_array($c) && 3 == sizeof($c) && 'Jill' == $c[0]->name_first && 'Joan' == $c[1]->name_first
        && 'JAMIE' == $c[2]->name_first) {
    ...
    $ch->Load('id=1');
    if ('Jill' !== $ch->name_first) {
        echo 'error in Loading Child<br>';
    }

    $p = $ch->person;
    if (!$p || 'John' != $p->name_first) {
    ...
    if ($p) {
        ...
        if (2 == sizeof($p->children) && 'JAMIE' == $p->children[1]->name_first) {

...
function CheckWS($conn)
{
    ...
    if (headers_sent()) {
        echo "<p><b>White space detected in adodb-$conn.inc.php or include file...</b></p>";
        //die();
    }
}
...
FROM `nuke_stories` `t1`, `nuke_authors` `t2`, `nuke_stories_cat` `t3`, `nuke_topics` `t4`
    ...
    echo "<h3>ADODB Version: $ADODB_vers Host: <i>$db->host</i> &nbsp; Database: <i>$db->database</i> &nbsp; PHP: $phpv $ext</h3>";
    ...
    echo '<i>date1</i> (1969-02-20) = '.$db->DBDate('1969-2-20');
    echo '<br><i>date1</i> (1999-02-20) = '.$db->DBDate('1999-2-20');
    echo '<br><i>date1.1</i> 1999 injection attack= '.$db->DBDate("'1999', ' injection attack '");
    echo '<br><i>date2</i> (1970-1-2) = '.$db->DBDate(24 * 3600).'<p>';
    echo '<i>ts1</i> (1999-02-20 13:40:50) = '.$db->DBTimeStamp('1999-2-20 1:40:50 pm');
    echo '<br><i>ts1.1</i> (1999-02-20 13:40:00) = '.$db->DBTimeStamp('1999-2-20 13:40');
    echo '<br><i>ts2</i> (1999-02-20) = '.$db->DBTimeStamp('1999-2-20');
    echo '<br><i>ts2</i> (1999-02-20) = '.$db->DBTimeStamp("'1999-2-20', 'injection attack'");
    echo '<br><i>ts3</i> (1970-1-2 +/- timezone) = '.$db->DBTimeStamp(24 * 3600);
    echo '<br> Fractional TS (1999-2-20 13:40:50.91): '.$db->DBTimeStamp($db->UnixTimeStamp('1999-2-20 13:40:50.91+1'));
    $dd = $db->UnixDate('1999-02-20');
    echo '<br>unixdate</i> 1999-02-20 = '.date('Y-m-d', $dd).'<p>';
    echo '<br><i>ts4</i> ='.($db->UnixTimeStamp('19700101000101') + 8 * 3600);
    echo '<br><i>ts5</i> ='.$db->DBTimeStamp($db->UnixTimeStamp('20040110092123'));
    echo '<br><i>ts6</i> ='.$db->UserTimeStamp('20040110092123');
    echo '<br><i>ts7</i> ='.$db->DBTimeStamp('20040110092123');
    flush();
    // mssql too slow in failing bad connection
    if (false && 'mssql' != $db->databaseType) {
        echo '<p>Testing bad connection. Ignore following error msgs:<br>';
        $db2 = ADONewConnection();
        $rez = $db2->Connect('bad connection');
        $err = $db2->ErrorMsg();
        echo "<i>Error='$err'</i></p>";
        if ($rez) {
    ...
    if ($create) {
        if (false && 'ibase' == $db->databaseType) {
            echo "<b>Please create the following table for testing:</b></p>$createtab</p>";
    ...
    echo '<p>Test select on empty table, FetchField when EOF, and GetInsertSQL</p>';
    ...
    flush();
    //$db->debug=true;
    echo '<p>Testing Commit: ';
    $time = $db->DBDate(time());
    if (!$db->BeginTrans()) {
        echo '<b>Transactions not supported</b></p>';
    ...
    if (1) {
        echo '<p>Testing MetaDatabases()</p>';
        print_r($db->MetaDatabases());

        echo '<p>Testing MetaTables() and MetaColumns()</p>';
        $a = $db->MetaTables();
        if (false === $a) {
            print '<b>MetaTables not supported</b></p>';
        } else {
            echo 'Array of tables and views: ';
            foreach ($a as $v) {
                print " ($v) ";
            }
            echo '</p>';
        ...
        if (false === $a) {
            ...
            echo '<p>Columns of ADOXYZ: <font size=1><br>';
            foreach ($a as $v) {
                print_r($v);
                echo '<br>';
            }
        ...
        echo '<p>Testing MetaIndexes</p>';

        $a = $db->MetaIndexes(('ADOXYZ'), true);
        if (false === $a) {
            print '<b>MetaIndexes not supported</b></p>';
        } else {
            echo '<p>Indexes of ADOXYZ: <font size=1><br>';
            adodb_pr($a);
            echo '</font>';
        }
        echo '<p>Testing MetaPrimaryKeys</p>';
...
*/
        ...
        */
        ...
        if (empty($begin_date) or empty($end_date) or $begin_date == $end_date) {
            ...
            echo "begin=$begin_date end=$end_date<p>";
        } else {
    ...
    case 'oci8po':
        ...
        */
        ...
        echo '<h4>Testing Blob: size='.strlen($s).'</h4>';
        ...
        echo '<h4>Testing Clob: size='.strlen($s).'</h4>';
        ...
        $s = '';
        $s2 = '';
        echo '<h4>Testing Foreign Keys</h4>';
...
*/
        ...
        echo '<h4>Testing Cursor Variables</h4>';
        ...
        if ($rs && !$rs->EOF) {
            ...
            echo '<b>Error in using Cursor Variables 1</b><p>';
        }
        if ($rs) {
            $rs->Close();
        }

        echo '<h4>Testing Stored Procedures for oci8</h4>';
        ...
        $rs = $db->Execute($stmt);
        if ($rs) {
            if ('Cinta Hati Malaysia' !== $a2) {
                print "<b>Stored Procedure Error: a2 = $a2</b><p>";
            } else {
                echo  "OK: a2=$a2<p>";
            }
        } else {
            echo '<b>Error in using Stored Procedure IN/Out Variables</b><p>';
        }
    ...
    //$db->debug=1;
    echo '<p>Testing Bulk Insert of 3 rows</p>';
  ...
  KEY `myindex` (`row1`,`row2`)
    ...
    } elseif (0 != $rs->RecordCount()) {
        echo '<p><b>Error on RecordCount. Should be 0. Was '.$rs->RecordCount().'</b></p>';
        print_r($rs->fields);
    }
    if ('odbc' !== $db->databaseType) {
        $rs = $db->Execute('select id,firstname,lastname,created,'.$db->random.' from ADOXYZ order by id');
        if ($rs) {
            if (50 != $rs->RecordCount()) {
                echo '<p><b>RecordCount returns '.$rs->RecordCount().', should be 50</b></p>';
    ...
    if (50 == $val1[0] and 1 == sizeof($val1) and 50 == $val2[0] and 1 == sizeof($val2)) {
        print '<p>GetRow returns ok</p>';
    } else {
        print_r($val);
        echo "<p><b>Fail: GetRow returns {$val2[0]}</b></p>";
    }

    echo '<p>FetchObject/FetchNextObject Test</p>';
    ...
    if ($rs) {
        ...
        while ($o = $rs->FetchNextObject()) { // calls FetchObject internally
            if (!is_string($o->FIRSTNAME) || !is_string($o->LASTNAME)) {
                print_r($o);
                echo '<p><b>Firstname is not string</b></p>';
                break;
            }
        }
    } else {
        echo '<p><b>Failed rs</b></p>';
        die('<p>ADOXYZ table cannot be read - die()');
    }
    $ADODB_FETCH_MODE = ADODB_FETCH_ASSOC;
    echo '<p>FetchObject/FetchNextObject Test 2</p>';
    //$db->debug=99;
    $rs = $db->Execute('select * from ADOXYZ');
    if (empty($rs->connection)) {
        print '<b>Connection object missing from recordset</b></br>';
    ...
    echo '<p>CacheSelectLimit  Test...</p>';
    $db->debug = 1;
    ...
    if ($rs && !$rs->EOF) {
        ...
        if ('Caroline' != trim($rs->fields['firstname'])) {
            echo Err('Error 2');
            print_r($rs->fields);
        }
    ...
    echo '<p>FETCH_MODE = ASSOC: Should get 1, Caroline</p>';
    ...
    if ($rs && !$rs->EOF) {
        ...
        if ('Caroline' != trim($rs->fields[$fname])) {
            Err('Error 2');
            print_r($rs->fields);
        }
        $rs->MoveNext();
    ...
    $ADODB_FETCH_MODE = ADODB_FETCH_NUM;
    echo '<p>FETCH_MODE = NUM: Should get 1, Caroline</p>';
    ...
    $db->debug = false;
    echo '<p>GetRowAssoc Upper: Should get 1, Caroline</p>';
    $rs = $db->SelectLimit('select id,firstname from ADOXYZ order by id', 1);
    if ($rs && !$rs->EOF) {
        $arr = $rs->GetRowAssoc();
        if (1 != $arr['ID']) {
        ...
        if ('Caroline' != trim($arr['FIRSTNAME'])) {
            Err('Error 2');
            print_r($arr);
        }
        $rs->MoveNext();
    ...
    echo '<p>GetRowAssoc Lower: Should get 1, Caroline</p>';
    $rs = $db->SelectLimit('select id,firstname from ADOXYZ order by id', 1);
    if ($rs && !$rs->EOF) {
    ...
    echo '<p>SelectLimit Distinct Test 1: Should see Caroline, John and Mary</p>';
    ...
    if ($rs && !$rs->EOF) {
        ...
        if ('John' != trim($rs->fields[1])) {
            Err('Error 2 (exp John), '.$rs->fields[1]);
        }
        $rs->MoveNext();
        if ('Mary' != trim($rs->fields[1])) {
    ...
    echo '<p>SelectLimit Test 2: Should see Mary, George and Mr. Alan</p>';
    ...
    if ($rs && !$rs->EOF) {
        ...
        $rs->MoveNext();
        if ('George' != trim($rs->fields[1])) {
            Err('Error 2 - No George, instead: '.$rs->fields[1]);
        }
        $rs->MoveNext();
        if ('Mr. Alan' != trim($rs->fields[1])) {
    ...
    echo '<p>SelectLimit Test 3: Should see Wai Hun and Steven</p>';
    $db->debug = 1;
    global $A;
    $A = 1;
    ...
    if ($rs && !$rs->EOF) {
        ...
        if ('Wai Hun' != trim($rs->fields[1])) {
            Err('Error 1 '.$rs->fields[1]);
        }
        $rs->MoveNext();
        if ('Steven' != trim($rs->fields[1])) {
    ...
    if (!$rs) {
        ...
        if (!$rs->Move(2)) {
            ...
            $rs->MoveFirst();
            if ('Caroline' != trim($rs->Fields('firstname'))) {
                echo "<p><b>$db->databaseType: MoveFirst failed -- probably cannot scroll backwards</b></p>";
            } else {
            ...
            $rs->Move(3);
            if ('George' != trim($rs->Fields('firstname'))) {
                echo '<p>'.$rs->Fields('id')."<b>$db->databaseType: Move(3) failed</b></p>";
            } else {
                print 'Move(3) OK<BR>';
            }

            $rs->Move(7);
            if ('Yat Sun' != trim($rs->Fields('firstname'))) {
                echo '<p>'.$rs->Fields('id')."<b>$db->databaseType: Move(7) failed</b></p>";
                print_r($rs);
            } else {
                print 'Move(7) OK<BR>';
            }
            ...
            $rs->MoveLast();
            if ('Steven' != trim($rs->Fields('firstname'))) {
                echo '<p>'.$rs->Fields('id')."<b>$db->databaseType: MoveLast() failed</b></p>";
                print_r($rs);
            } else {
            ...
            if ($rs->canSeek) {
                $rs->Move(3);
                if ('George' != trim($rs->Fields('firstname'))) {
                    echo '<p>'.$rs->Fields('id')."<b>$db->databaseType: Move(3) after MoveLast failed</b></p>";
                } else {
                    print 'Move(3) after MoveLast() OK<BR>';
                }
            }

            echo '<p>Empty Move Test';
    ...
    //	$db->debug=true;
    echo '<p>Testing ADODB_FETCH_ASSOC and concat: concat firstname and lastname</p>';

    $save = $ADODB_FETCH_MODE;
    $ADODB_FETCH_MODE = ADODB_FETCH_ASSOC;
    if ('postgres' == $db->dataProvider) {
    ...
    echo '<hr />Testing GetArray() ';
    //$ADODB_FETCH_MODE = ADODB_FETCH_ASSOC;

    $rs = $db->Execute('select * from ADOXYZ order by id');
    if ($rs) {
        $arr = $rs->GetArray(10);
        if (10 != sizeof($arr) || 'John' != trim($arr[1][1]) || 'Lim' != trim($arr[1][2])) {
    ...
    echo 'Testing FetchNextObject for 1 object ';
    ...
    if ($rs) {
        $arr = $rs->GetAssoc();
        //print_r($arr);
        if (empty($arr['See']) || 'Wai Hun' != trim(reset($arr['See']))) {
    ...
    if ($arr) {
        //print_r($arr);
        if (empty($arr['See']) || 'Wai Hun' != trim($arr['See'])) {
            print $arr['See'].' &nbsp; <b>ERROR</b><br>';
        } else {
            print ' OK 2<BR>';
        }
    ...
    if (50 != reset($rs->fields)) {
        echo Err("$cnt wrong for Prepare/CacheGetOne");
    }

    for ($loop = 0; $loop < 1; ++$loop) {
        echo 'Testing GetMenu() and CacheExecute<BR>';
        $db->debug = true;
        ...
        $rs = $db->CacheExecute(4, 'select distinct firstname,lastname from ADOXYZ');
        if ($rs) {
            echo ' Multiple, Alan and George selected: '.$rs->GetMenu('menu', array('Alan', 'George'), false, true);
            if (empty($rs->connection)) {
                print '<b>Connection object missing from recordset</b></br>';
            }
        } else {
    ...
    if (empty($rs1) || empty($rs2) || 2 != $rs1->fields[0] || 4 != $rs2->fields[0]) {
        $a = $rs1->fields[0];
        $b = $rs2->fields[0];
        echo "<p><b>Error in multiple recordset test rs1=$a rs2=$b (should be rs1=2 rs2=4)</b></p>";
    } else {
    ...
    if ('notused' != substr($db->dataProvider, 0, 3)) { // used to crash ado
        $sql = 'select firstnames from ADOXYZ';
        echo "<p>Testing execution of illegal statement: <i>$sql</i></p>";
        if (false === $db->Execute($sql)) {
            echo '<p>This returns the following ErrorMsg(): <i>'.$db->ErrorMsg().'</i> and ErrorNo(): '.$db->ErrorNo().'</p>';
        } else {
    ...
    for ($i = 0; $i < $rs->FieldCount(); ++$i) {
        $fld = $rs->FetchField($i);
        echo '<br> Field name is '.$fld->name;
        echo ' '.$rs->Fields($fld->name);
    }

    echo '<p>BOTH TEST 2<br>';
    if ('ado' == $db->dataProvider) {
        echo '<b>ADODB_FETCH_BOTH not supported</b> for dataProvider='.$db->dataProvider.'<br>';
    } else {
    ...
    echo '<p>ASSOC Test of SelectLimit<br>';
    $ADODB_FETCH_MODE = ADODB_FETCH_ASSOC;
    ...
    while ($rs && !$rs->EOF) {
        ++$cnt;
        if (!isset($rs->fields['firstname'])) {
            echo '<br><b>ASSOC returned numeric field</b></p>';
            break;
        }
        $rs->MoveNext();
    }
    ...
    if ($db->sysDate) {
        ...
        if (ADORecordSet::UnixDate(date('Y-m-d')) != $rs->UnixDate($rs->fields[0])) {
            echo "<p><b>Invalid date {$rs->fields[0]}</b></p>";
        } else {
        ...
        echo '<p><b>$db->sysDate not defined</b></p>';
    }
    ...
    echo '<p>Test SQLDate: '.htmlspecialchars($sql).'</p>';
    ...
    if (!$ret) {
        ...
        if (1 != $rs->RecordCount()) {
            $cnt = $rs->RecordCount();
            rs2html($rs);
            echo "<b>Error - Replace failed, count=$cnt</b><p>";
        }
    ...
    echo "test A return value=$ret (2 expected) <p>";
    ...
    $ret = $db->Replace('ADOXYZ',
        ...
        $autoq = true);
    if (1 != $ret) {
        if ('ibase' == $db->dataProvider && 2 == $ret); else {
            print '<b>Replace failed: </b>';
        }
    }
    echo "test B return value=$ret (1 or if ibase then 2 expected) <p>";

    echo '<h3>rs2rs Test</h3>';
    ...
    if ($pear) {
        ...
        if ($rs && !$rs->EOF) {
            while ($arr = $rs->fetchRow()) {
                ...
                if ($arr[0] != $i) {
                    print_r($arr);
                    echo '<p><b>PEAR DB emulation error 1.</b></p>';
                    $pear = false;
                    break;
                }
            }
        ...
        if ($i != $db->GetOne('select count(*) from ADOXYZ where id>0 and id<10')) {
            echo "<p><b>PEAR DB emulation error 1.1 EOF ($i)</b></p>";
            $pear = false;
        }
        ...
        if ($rs && !$rs->EOF) {
            while (!is_object($rs->fetchInto($arr))) {
                ...
                //		print "$i ";print_r($arr);
                if ($arr[0] != $i2) {
                    echo '<p><b>PEAR DB emulation error 2.</b></p>';
                    $pear = false;
                    break;
                }
            }
        ...
        if ($i2 != $i + $top) {
            echo "<p><b>PEAR DB emulation error 2.1 EOF (correct=$i+$top, actual=$i2)</b></p>";
            $pear = false;
        }
    }
    if ($pear) {
    ...
    echo '<p>Test MetaTables again with SetFetchMode()</p>';
    $ADODB_FETCH_MODE = ADODB_FETCH_ASSOC;
    $db->SetFetchMode(ADODB_FETCH_ASSOC);
    ...
    echo '<p>Testing Bad Connection</p>';
    flush();

    if (true || PHP_VERSION < 5) {
        if ('odbtp' == $db->dataProvider) {
    ...
    if (empty($nocountrecs) && false == $ADODB_COUNTRECS) {
...
function adodb_test_err($dbms, $fn, $errno, $errmsg, $p1 = false, $p2 = false)
{
    ...
    $ERRNO = $errno;
    ++$TESTERRS;
    echo "<i>** $dbms ($fn): errno=$errno &nbsp; errmsg=$errmsg ($p1,$p2)</i><br>";
}
...
if ('TIGRESS' == @$_SERVER['COMPUTERNAME']) {
    CheckWS('mysqlt');
    CheckWS('postgres');
    CheckWS('oci8po');
    ...
    CheckWS('proxy');
    CheckWS('fbsql');
    echo 'White Space Check complete<p>';
}
if (0 == sizeof($_GET)) {
    $testmysql = true;
}

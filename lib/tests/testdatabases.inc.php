...
if (ADODB_FETCH_DEFAULT != $ADODB_FETCH_MODE) {
...
if (!empty($testpostgres)) {
    ...
    echo "<h1>Connecting $db->databaseType...</h1>";
...
if (!empty($testaccess) && !empty($testado)) { // ADO ACCESS
    ...
    if ($db->PConnect($myDSN, '', '', '')) {
        echo 'ADO version='.$db->_connectionID->version.'<br>';
        testdb($db, 'create table ADOXYZ (id int, firstname char(24), lastname char(24),created datetime)');
    } else {
...
if (!empty($testmysql)) { // MYSQL
    if (PHP_VERSION >= 5 || 'localhost' == $_SERVER['HTTP_HOST']) {
        $server = 'localhost';
    } else {
        $server = 'mangrove';
    }
...
if (!empty($testmssql) && !empty($testado)) { // ADO ACCESS MSSQL -- thru ODBC -- DSN-less
    ...
    //$db->debug=1;
    echo "<h1>Connecting DSN-less $db->databaseType...</h1>";
...
if (!empty($testmssql) && !empty($testado)) { // ADO ACCESS MSSQL with OLEDB provider
    $db = ADONewConnection('ado_mssql');
    echo "<h1>Connecting DSN-less OLEDB Provider $db->databaseType...</h1>";
    //$db->debug=1;
